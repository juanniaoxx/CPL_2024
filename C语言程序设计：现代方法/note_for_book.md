...
> # 几个不常用的宏
>
> `#error [message]` 展开这条宏的时候编译器会立即停止编译
>
> ```c 
> #if INT_MAX < 100000
...
> `#line`用来改变程序行的编号方法。
>
> `#pragma 指令` 要求编译器执行某些题数操作
>
> ```c
> #pragma data(heap_size => 1000, stack_size => 2000)
> ```
>
> `_Pragma(字面串)` 与`#pragma`
>
> ```c
...
> // 是GCC支持的一种编译提示，如果指定文件笔当前文件还要新，则会给出警告。
> // #是运算符，会导致x被串化
> //而_Pragma会去串化
> ```
...
> >
> > ```c
> > #include <xxx.h> // 通常用于包含C库文件
> > #include "xxx.h" // 通常用于包含自己写的头文件
> > ```
> >
> > > [!Important]
> > >
...
> > > [!Important]
> > >
> > > 使用`ifndef` 和`endif`避免一个头文件被重复包含	
> > >
> > > ```c
> > > #ifndef  BOOLEAN_H
> > > #defien BOOLEAN_H
...
>     <div>
>         makefile对于C/C++的重要是不言而喻的，虽然有Cmake可以缩短写makefile，但学习写makefile仍然是重要且必须的！
>         <a href="https://seisman.github.io/how-to-write-makefile/overview.html">推荐学习</a>
>     </div>
> </details>
>
> 常见链接错误
>
> - 拼写错误导致链接器无法识别对于的函数/变量
> - 缺少文件，缺失对应函数/变量的依赖
> - 缺少库，使用了未被包含的库函数
>
> > ##### 在程序外定义宏
